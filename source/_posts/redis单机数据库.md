...
- 设置生存和过期时间
  ...
  ![image-20211110165635236](redis单机数据库/image-20211110165635236.png)

- 带有生存和过期时间的数据库

  ![image-20211110165743757](redis单机数据库/image-20211110165743757.png)
...
- 过期键删除:
  ...
  - 惰性删除
    ...
    - db.c的expireIfNeeded()函数对于所有的输入键检查是否过期,过期的键直接删除
      ...
      ![image-20211111154024925](redis单机数据库/image-20211111154024925.png)
    ...
    - AOF对于过期键处理
      ...
      - 写入时如果键过期但没有删除,不会影响,当(惰性或者定期)删除的时候追加一条DEL
        - ![image-20211111214557807](redis单机数据库/image-20211111214557807.png)
    ...
    - 复制对于过期键处理
      ...
      - ![image-20211111220239415](redis单机数据库/image-20211111220239415.png)
      
        ![image-20211111220255616](redis单机数据库/image-20211111220255616.png)
